...
interface CourseJson {
  course?: { name: string; chapters?: Array<{ name: string; duration: string }> }
}
...
export default function LearnPage() {
  ...
  useEffect(() => {
    if (isNaN(cid)) return
    fetch(`/api/courses/${cid}`).then((r) => r.json()).then(setCourse).catch(() => setCourse(null))
    fetch(`/api/progress?courseCid=${cid}`).then((r) => r.json()).then((d) => setCompletedChapters(d.completedChapters || [])).catch(() => setCompletedChapters([]))
  }, [cid])
  ...
  const markComplete = async () => {
    try {
      await axios.post('/api/progress', { courseCid: cid, chapterIndex: selectedChapter })
      setCompletedChapters((prev) => (prev.includes(selectedChapter) ? prev : [...prev, selectedChapter]))
    } catch {}
  }
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="sm" asChild>
          <Link href={`/workspace/courses/${cid}`}><ChevronLeft className="w-4 h-4 mr-1" />Back to course</Link>
        </Button>
      </div>
          ...
          <ul className="space-y-1">
            {chapters.map((ch, i) => (
              <li key={i}>
                ...
                >
                  {completedChapters.includes(i) && <Check className="w-4 h-4 text-green-600 shrink-0" />}
        ...
        <div className="lg:col-span-2 border rounded-lg p-6 bg-white min-h-[400px]">
          {currentChapter ? (
            <>
              ...
              <p className="text-sm text-gray-500 mt-1">{currentChapter.duration}</p>
              <div className="mt-6 prose prose-sm max-w-none">
                {loadingContent ? (
                  <div className="flex items-center gap-2 text-gray-500"><Loader2 className="w-5 h-5 animate-spin" /> Loading content...</div>
                ) : (
                  <div className="whitespace-pre-wrap text-gray-700">{chapterContent || 'Click a chapter to load content.'}</div>
                )}
              </div>
              {!loadingContent && chapterContent && (
                <Button onClick={markComplete} className="mt-6 bg-purple-600 hover:bg-purple-700" disabled={completedChapters.includes(selectedChapter)}>
                  {completedChapters.includes(selectedChapter) ? <Check className="w-4 h-4 mr-2" /> : null}
